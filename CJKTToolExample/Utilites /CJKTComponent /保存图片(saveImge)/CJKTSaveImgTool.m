...
#import "CJKTSaveImgTool.h"
#import <AssetsLibrary/AssetsLibrary.h>
#import <Photos/Photos.h>
@interface CJKTSaveImgTool ()
@property (nonatomic, copy) void(^completion)(NSString *title);
@end
@implementation CJKTSaveImgTool

+ (instancetype)sharedInstance {
    static CJKTSaveImgTool *_sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _sharedInstance = [[CJKTSaveImgTool alloc] init];
...
/**
 保存图片到相册
 */
- (void)cjkt_saveImageToUserPhotoAlbum:(UIImage *)image completion:(void(^)(BOOL success, NSError * _Nullable error))completion {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if(status == PHAuthorizationStatusDenied) //用户拒绝（可能是之前拒绝的，有可能是刚才在系统弹框中选择的拒绝）
            {
                if (lastStatus == PHAuthorizationStatusNotDetermined) {
                    ...
                    NSLog(@"保存失败");
                    self.completion(@"保存失败！请在系统设置中开启访问相册权限");
                    return;
                }
            ...
            {
                 NSLog(@"系统原因，无法访问相册");
                 self.completion(@"保存失败!系统原因，无法访问相册");
            }
        });
    }];
}
