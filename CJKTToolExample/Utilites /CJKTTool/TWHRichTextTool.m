...
+ (NSMutableAttributedString *)changeFontAndColor:(UIFont *)font Color:(UIColor *)color TotalString:(NSString *)totalString SubStringArray:(NSArray *)subArray AndOptions:(NSInteger)options andchangeCharacterSpaceWithLineSpace:(CGFloat)space spaceStringArray:(NSArray *)spaceArr{
    ...
    for (NSString *rangeStr in spaceArr) {
        NSRange range = [totalString rangeOfString:rangeStr options:options];
        long number = space;
        CFNumberRef num = CFNumberCreate(kCFAllocatorDefault,kCFNumberSInt8Type,&number);
        [attributedStr addAttribute:(id)kCTKernAttributeName value:(__bridge id)num range:range];
        CFRelease(num);
    }
